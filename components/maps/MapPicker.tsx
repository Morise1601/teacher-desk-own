...
const parseCityState = (fullAddress: string) => {
  if (!fullAddress) return { city: '', state: '' };
  const parts = fullAddress.split(',').map(p => p.trim()).filter(Boolean);
  let city = '';
  let state = '';
  
  const indiaIndex = parts.findIndex(p => p.toLowerCase() === 'india');
  
  if (indiaIndex !== -1) {
    let statePart = parts[indiaIndex - 1] || '';
    let cityPart = parts[indiaIndex - 2] || '';
    
    statePart = statePart.replace(/\d+/g, '').trim();
    
    if (!statePart && indiaIndex >= 2) {
      statePart = parts[indiaIndex - 2].replace(/\d+/g, '').trim();
      cityPart = parts[indiaIndex - 3] || '';
    }
    
    state = statePart;
    city = cityPart;
  } else if (parts.length >= 2) {
    state = parts[parts.length - 2].replace(/\d+/g, '').trim();
...
export default function MapPicker({ onLocationSelect, onSelect, onClose, title, defaultAddress }: MapPickerProps) {
  ...
  useEffect(() => {
    ...
    if (staticCountries && staticCountries.length > 0) {
      const india = staticCountries.find(c => c.name.common === 'India');
      if (india) {
    ...
    if (defaultAddress) {
      ...
      const parsed = parseCityState(defaultAddress);
      if (parsed.state) {
      ...
      const geocodeAddress = async () => {
        try {
          const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(defaultAddress)}&format=json&limit=1`);
          if (res.ok) {
          ...
          console.error("Failed to geocode default address:", err);
        }
      };
  ...
  const fetchInstitutions = async (city: string, state: string, country: string) => {
    ...
    try {
      ...
      // 1. Get city center coordinates in a single Nominatim geocode call
      const cityGeocode = await safeFetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(city + ', ' + state + ', ' + country)}&format=json&limit=1`);
      if (cityGeocode && Array.isArray(cityGeocode) && cityGeocode.length > 0) {
          lat = parseFloat(cityGeocode[0].lat);
          lon = parseFloat(cityGeocode[0].lon);
          setPosition([lat, lon]);
      }

      // 2. Query Overpass API for schools/colleges/universities around these coordinates (20km radius)
      const opQuery = `[out:json][timeout:25];(node["amenity"~"university|college|school|kindergarten|language_school|music_school"](around:20000,${lat},${lon});way["amenity"~"university|college|school|kindergarten|language_school|music_school"](around:20000,${lat},${lon});node["building"~"university|college|school"](around:20000,${lat},${lon});way["building"~"university|college|school"](around:20000,${lat},${lon});node["office"="educational_institution"](around:20000,${lat},${lon}););out center;`;
