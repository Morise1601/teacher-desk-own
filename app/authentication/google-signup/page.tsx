...
import { 
  ...
  FaGraduationCap, FaBriefcase, FaLock, FaMapMarkedAlt, FaGlobe, FaBuilding
} from 'react-icons/fa';
...
export default function GoogleSignupPage() {
  ...
  const handleTeacherSubmit = async (e: React.FormEvent) => {
    ...
    if (!teacherData.phone || teacherData.phone.length !== 10) newErrors.phone = true;
    if (!teacherData.gender) newErrors.gender = true;
    if (!teacherData.dob) newErrors.dob = true;
    if (!teacherData.qualification || (teacherData.qualification === 'other' && !customQual)) newErrors.qualification = true;
    if (!teacherData.specialization || (teacherData.specialization === 'other' && !customSpec)) newErrors.specialization = true;
    if (!teacherData.experience) newErrors.experience = true;
  ...
  const handleInstitutionSubmit = async (e: React.FormEvent) => {
    ...
    const newErrors: Record<string, boolean> = {};
    if (!instData.name) newErrors.name = true;
    if (!instData.type) newErrors.type = true;
    if (!instData.address) newErrors.address = true;
    if (!instData.email) newErrors.email = true;
    if (!instData.phone || instData.phone.length !== 10) newErrors.phone = true;

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      toast.error("Please fill all required institution fields correctly.");
      return;
    }
    ...
    <div className="min-h-screen w-full bg-white flex relative overflow-hidden select-none">
      ...
      <div className="w-full relative z-10 flex flex-col items-center justify-center p-6 md:p-12 overflow-y-auto">
        <AnimatePresence mode="wait">
          ...
          {/* STEP 2A: Teacher Registration Form */}
          {role === 'teacher' && (
            <motion.div
            ...
            >
              ...
              <form onSubmit={handleTeacherSubmit} className="space-y-4 authForms">
                ...
                <button
                  type="submit"
                  className="w-full h-10 bg-[var(--color-primary)] text-white font-bold text-xs tracking-widest uppercase rounded-md shadow-md hover:shadow-lg transition-all mt-4"
                >
          ...
          {role === 'institution' && (
            ...
            >
              ...
              <form onSubmit={handleInstitutionSubmit} className="space-y-4 authForms">
